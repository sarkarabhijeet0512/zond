...
func NewStateProcessorV1(config *params.ChainConfig, getHashFunc vm.GetHashFunc) *StateProcessorV1 {
	return &StateProcessorV1{
		config:      config,
		getHashFunc: getHashFunc,
	}
}

func (p *StateProcessorV1) ProcessGenesisPreState(preState *protos.PreState, b *block.Block, db *db.DB, statedb *state.StateDB) error {
...
func (p *StateProcessorV1) ProcessGenesis(b *block.Block, statedb *state.StateDB, stateContext *state2.StateContext, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	blockContext := NewEVMBlockContextV1(header, p.getHashFunc, b.Minter())
	...
	for i, protoTx := range b.ProtocolTransactions() {
		...
		case *protos.ProtocolTransaction_CoinBase:
			...
			statedb.SetTxContext(coinBaseTxHash, 0)
	...
	for i, protoTx := range b.Transactions() {
		...
		case *protos.Transaction_Transfer:
			...
			receipt, err = applyTransactionV1(msg, gp, statedb, blockNumber, blockHash, tx, usedGas, vmenv)
			if err != nil {
...
func (p *StateProcessorV1) Process(b, parentBlock *block.Block, statedb *state.StateDB, stateContext *state2.StateContext, slotValidatorsMetaData *metadata.SlotValidatorsMetaData, isFinalizedState bool, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	var (
		protocolTxReceipts types.Receipts
		txReceipts         types.Receipts
		usedGas            = new(uint64)
	...
	for i, protoTx := range b.ProtocolTransactions() {
		...
		case *protos.ProtocolTransaction_CoinBase:
			...
			coinBaseTxHash := coinBaseTx.TxHash(coinBaseTx.GetSigningHash(b.BlockSigningHash()))
			statedb.SetTxContext(coinBaseTxHash, i)
			receipt, err = applyCoinBaseTransaction(statedb, stateContext, blockNumber, blockHash, coinBaseTx)
			if err != nil {
				return nil, nil, 0, fmt.Errorf("could not apply coinbase tx %d [%v]: %w", 0, coinBaseTxHash, err)
			}
...
func (p *StateProcessorV1) Commit(s *state2.StateContext, b *block.Block, protocolTxReceipts, TxReceipts types.Receipts, trieRoot common.Hash, isFinalizedState bool) error {
...
func applyTransactionV1(msg types.Message, gp *GasPool, statedb *state.StateDB, blockNumber *big.Int, blockHash common.Hash, tx transactions.TransactionInterface, usedGas *uint64, evm *vm.EVM) (*types.Receipt, error) {
...
func applyCoinBaseTransaction(statedb *state.StateDB, stateContext *state2.StateContext, blockNumber *big.Int, blockHash common.Hash, tx *transactions.CoinBase) (*types.Receipt, error) {
	...
	if err := stateContext.ProcessBlockProposerFlag(tx.PK(), accountState.Balance()); err != nil {
...
func applyAttestTransaction(statedb *state.StateDB, stateContext *state2.StateContext, blockNumber *big.Int, blockHash common.Hash, tx *transactions.Attest, attestorReward uint64) (*types.Receipt, error) {
	...
	if err := stateContext.ProcessAttestorsFlag(tx.PK(), accountState.Balance()); err != nil {
...
func ValidateAttestTx(tx *transactions.Attest, statedb *state.StateDB, slotValidatorsMetaData *metadata.SlotValidatorsMetaData, partialBlockSigningHash common.Hash, slotNumber uint64, parentSlotNumber uint64) error {
	...
	if accountState.Balance().Uint64() < config.GetDevConfig().StakeAmount {
		/*
		...
		*/
		if parentEpoch == currentEpoch {
			return fmt.Errorf(errmsg.TXInsufficientStakeBalance,
				"attest", tx.Hash(), signerAddr, accountState.Balance())
		} else {
			if accountState.Balance().Uint64() < config.GetDevConfig().StakeAmount {
				return fmt.Errorf(errmsg.TXInsufficientStakeBalance,
					"attest", tx.Hash(), signerAddr, accountState.Balance())
