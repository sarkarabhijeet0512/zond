...
import (
	...
	"github.com/theQRL/zond/ethdb/memorydb"
	...
	onAccount := func(key []byte, val []byte, write bool, delete bool) error {
		...
		if accMarker == nil || !bytes.Equal(account[:], accMarker) {
			...
			if !write {
				...
				data := SlimAccountRLP(acc.Nonce, acc.Balance, acc.StakeBalance, acc.PendingStakeBalance, acc.Root, acc.CodeHash, acc.OTSBitfield)
				dataLen = len(data)
