...
import (
	...
	"github.com/theQRL/zond/core/state/snapshot"
...
func (s *StateDB) GetOTSBitfield(addr common.Address) [][8]byte {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
		return stateObject.OTSBitfield()
	}
...
func (s *StateDB) SetStakeBalance(addr common.Address, amount *big.Int) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetStakeBalance(amount)
	}
}

func (s *StateDB) SetPendingStakeBalance(addr common.Address, amount *big.Int) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetPendingStakeBalance(amount)
	}
}
...
func (s *StateDB) SetOTSBitfield(addr common.Address, otsIndex uint64, revert bool) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetOTSBitfield(otsIndex, revert)
	}
}
...
func (s *StateDB) updateStateObject(obj *stateObject) {
	...
	if s.snap != nil {
		s.snapAccounts[obj.addrHash] = snapshot.SlimAccountRLP(obj.data.Nonce, obj.data.Balance, obj.data.StakeBalance, obj.data.PendingStakeBalance, obj.data.Root, obj.data.CodeHash, obj.data.OTSBitfield)
	}
}
...
func (s *StateDB) Copy() *StateDB {
	...
	for addr := range s.journal.dirties {
		// As documented [here](https://github.com/theQRL/zond/pull/16485#issuecomment-380438527),
