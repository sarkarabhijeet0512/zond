...
// 	return ln
// }
func NewLocalNode(db *DB, privkey *crypto2.DilithiumPrivateKey) *LocalNode {
...
func (ln *LocalNode) sign() {
	...
	if err := SignV4(&r, ln.key); err != nil {
		panic(fmt.Errorf("znode: can't sign record: %v", err))
	}
	n, err := New(ValidSchemes, &r)
	if err != nil {
		panic(fmt.Errorf("znode: can't verify local record: %v", err))
	}
	ln.cur.Store(n)
