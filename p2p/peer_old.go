...
func newPeerV1(conn network.Stream, inbound bool, chain *chain.Chain,
	filter *bloom.BloomFilter, mr *MessageReceiptV1,
	...
	addPeerToPeerList chan *PeerIPWithPLData,
	blockAndPeerChan chan *BlockAndPeer,
	messagePriority map[protos.LegacyMessage_FuncName]uint64) *PeerV1 {
	p := &PeerV1{
		stream:                      conn,
		inbound:                     inbound,
		chain:                       chain,
	...
	log.Info("New PeerV1 connected ", p.stream.Conn().RemoteMultiaddr())
	return p
}

func (p *PeerV1) ID() string {
	return p.id
}

func (p *PeerV1) IP() string {
	return p.ip
}

func (p *PeerV1) ChainState() *protos.NodeChainState {
	return p.chainState
}

func (p *PeerV1) GetTotalStakeAmount() []byte {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *PeerV1) GetEpochToBeRequested() uint64 {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *PeerV1) IncreaseEpochToBeRequested() {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *PeerV1) UpdateEpochToBeRequested(epoch uint64) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
//func (p *PeerV1) GetNodeHeaderHashWithTimestamp() *NodeHeaderHashWithTimestamp {
//	return p.nodeHeaderHashWithTimestamp
//}

func (p *PeerV1) updateCounters() {
...
func (p *PeerV1) SendEBHReq(epoch uint64, finalizedHeaderHash []byte) error {
...
func (p *PeerV1) Send(msg *MsgV1) error {
...
func (p *PeerV1) ReadMsg() (msg *MsgV1, size uint32, err error) {
...
func (p *PeerV1) monitorChainState() {
	p.wg.Add(1)
	defer p.wg.Done()
	for {
		...
		case <-time.After(30 * time.Second):
			...
			if delta > int64(p.config.User.ChainStateTimeout) {
				log.Warn("Disconnecting PeerV1 due to Ping Timeout",
					" delta ", delta,
...
func (p *PeerV1) handle(msg *MsgV1) error {
	/*
	...
	*/
	...
	case protos.LegacyMessage_PL:
		...
		if p.isPLShared {
			log.Debug("PeerV1 list already shared before")
			return nil
		}
	...
	case protos.LegacyMessage_FB:
		...
		log.Info("Fetch Block Request",
			...
			" PeerV1 ", p.stream.Conn().RemoteMultiaddr())

		b, err := p.chain.GetBlock(blockHeaderHash)
		if err != nil {
			log.Info("Disconnecting PeerV1, as GetBlock returned nil")
	...
	case protos.LegacyMessage_PB:
		...
		if pbData.Block == nil {
			log.Info("Disconnecting PeerV1, as no block sent for Push Block")
			return errors.New("peer protocol error")
		}
	...
	case protos.LegacyMessage_P2P_ACK:
		...
		if p.bytesSent < 0 {
			log.Warn("Disconnecting PeerV1 due to negative bytes sent",
...
func (p *PeerV1) HandleBlockForAttestation(pbBlock *protos.Block, signature []byte) {
...
func (p *PeerV1) HandleBlock(pbBlock *protos.Block) {
...
func (p *PeerV1) HandleTransaction(msg *MsgV1, txData *protos.Transaction) error {
...
func (p *PeerV1) HandleAttestTransaction(msg *MsgV1, txData *protos.ProtocolTransactionData) error {
	pbData := txData.Tx
...
func (p *PeerV1) HandleChainState(nodeChainState *protos.NodeChainState) {
	p.chainState = nodeChainState
	p.chainState.Timestamp = p.ntp.Time()
}

func (p *PeerV1) SendFetchBlock(blockHeaderHash common.Hash) error {
	log.Info("Fetching",
...
func (p *PeerV1) handshake() {
	p.SendPeerList()
	// p.SendVersion()
	p.SendSync()
}

func (p *PeerV1) run() (remoteRequested bool) {
	p.handshake()
	go p.readLoop()
...
loop:
	...
	log.Info("PeerV1 routine closed for ", p.stream.Conn().RemoteMultiaddr())
