...
type (
	whoareyouAuthData struct {
		...
		RecordSeq uint64   // highest known znr sequence of requester
	}
...
var (
	...
	errNoRecord            = errors.New("expected znr in handshake but none sent")
...
// node.
func (c *Codec) Encode(id znode.ID, addr string, packet Packet, challenge *Whoareyou) ([]byte, Nonce, error) {
...
func (c *Codec) EncodeRaw(id znode.ID, head Header, msgdata []byte) ([]byte, error) {
	c.writeHeaders(&head)
...
func (c *Codec) makeHeader(toID znode.ID, flag byte, authsizeExtra int) Header {
	var authsize int
	switch flag {
	case flagMessage:
...
func (c *Codec) encodeRandom(toID znode.ID) (Header, []byte, error) {
...
func (c *Codec) encodeWhoareyou(toID znode.ID, packet *Whoareyou) (Header, error) {
...
func (c *Codec) encodeHandshakeHeader(toID znode.ID, addr string, challenge *Whoareyou) (Header, *session, error) {
...
func (c *Codec) makeHandshakeAuth(toID znode.ID, addr string, challenge *Whoareyou) (*handshakeAuthData, *session, error) {
...
func (c *Codec) encodeMessageHeader(toID znode.ID, s *session) (Header, error) {
	head := c.makeHeader(toID, flagMessage, 0)
...
func (c *Codec) Decode(input []byte, addr string) (src znode.ID, n *znode.Node, p Packet, err error) {
	if len(input) < minPacketSize {
		return znode.ID{}, nil, nil, errTooShort
	}
...
func (c *Codec) decodeHandshakeMessage(fromAddr string, head *Header, headerData, msgData []byte) (n *znode.Node, p Packet, err error) {
...
func (c *Codec) decodeHandshake(fromAddr string, head *Header) (n *znode.Node, auth handshakeAuthData, s *session, err error) {
...
func (c *Codec) decodeHandshakeRecord(local *znode.Node, wantID znode.ID, remote []byte) (*znode.Node, error) {
	node := local
	if len(remote) > 0 {
		var record znr.Record
		if err := rlp.DecodeBytes(remote, &record); err != nil {
			return nil, err
		}
		if local == nil || local.Seq() < record.Seq() {
			n, err := znode.New(znode.ValidSchemes, &record)
			if err != nil {
...
func (h *Header) mask(destID znode.ID) cipher.Stream {
