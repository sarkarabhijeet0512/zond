...
import (
	...
	zondcrypto "github.com/libp2p/go-libp2p/core/crypto"
...
func EncodePubkey(key *crypto2.DilithiumPrivateKey) []byte {
	// switch key.Curve {
	// case crypto.S256():
...
// 	}
// }
func makeIDSignature(hash hash.Hash, key *crypto2.DilithiumPrivateKey, challenge, ephkey []byte, destID znode.ID) ([]byte, error) {
	input := idNonceHash(hash, challenge, ephkey, destID)
	// switch {
	// case crypto.S256():
	idsig, err := key.Sign(input)
	// idsig, err := crypto.Sign(input, key)
	if err != nil {
		return nil, err
	}
...
// 	return &sec
// }
func deriveKeys(hash hashFn, priv *crypto2.DilithiumPrivateKey, pub *zondcrypto.PubKey, n1, n2 znode.ID, challenge []byte) *session {
	const text = "discovery v5 key agreement"
	var info = make([]byte, 0, len(text)+len(n1)+len(n2))
	info = append(info, text...)
	info = append(info, n1[:]...)
...
// 	return sec
// }
func ecdh(privkey *crypto2.DilithiumPrivateKey, pubkey *zondcrypto.PubKey) []byte {
	// privkeyBytes, _ := privkey.Bytes()
	// secX, secY := pubkey.ScalarMult(pubkey.X, pubkey.Y, privkeyBytes)
	// if secX == nil {
	// 	return nil
	// }
	sec := make([]byte, 33)
