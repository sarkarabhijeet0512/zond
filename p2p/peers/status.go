...
func (p *Status) BestFinalized(maxPeers int, ourFinalizedEpoch types.Epoch) (types.Epoch, []peer.ID) {
	...
	for _, pid := range connected {
		...
		if err == nil && peerChainState != nil && types.Epoch(peerChainState.FinalizedEpoch) >= ourFinalizedEpoch {
			finalizedEpochVotes[types.Epoch(peerChainState.FinalizedEpoch)]++
			pidEpoch[pid] = types.Epoch(peerChainState.FinalizedEpoch)
...
func (p *Status) HighestEpoch() types.Epoch {
	...
	for _, peerData := range p.store.Peers() {
		if peerData != nil && peerData.ChainState != nil && types.Slot(peerData.ChainState.HeadSlot) > highestSlot {
