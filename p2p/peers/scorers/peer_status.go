...
func (s *PeerStatusScorer) score(pid peer.ID) float64 {
	...
	if types.Slot(peerData.ChainState.HeadSlot) < s.ourHeadSlot {
		return score
	}
...
func (s *PeerStatusScorer) SetPeerStatus(pid peer.ID, chainState *pb.Status, validationError error) {
	...
	if chainState != nil && types.Slot(chainState.HeadSlot) > s.highestPeerHeadSlot {
		s.highestPeerHeadSlot = types.Slot(chainState.HeadSlot)
	}
}
