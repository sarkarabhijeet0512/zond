...
type NodeDialer interface {
	Dial(context.Context, *znode.Node) (net.Conn, error)
}
...
func (t tcpDialer) Dial(ctx context.Context, dest *znode.Node) (net.Conn, error) {
	return t.d.DialContext(ctx, "tcp", nodeAddr(dest).String())
}

func nodeAddr(n *znode.Node) net.Addr {
...
type dialScheduler struct {
	...
	ctx         context.Context
	nodesIn     chan *znode.Node
	doneCh      chan *dialTask
	addStaticCh chan *znode.Node
	remStaticCh chan *znode.Node
	addPeerCh   chan *conn
	remPeerCh   chan *conn

	// Everything below here belongs to loop and
	// should only be accessed by code on the loop goroutine.
	dialing   map[znode.ID]*dialTask // active tasks
	peers     map[znode.ID]struct{}  // all connected peers
...
type dialSetupFunc func(net.Conn, connFlag, *znode.Node) error

type dialConfig struct {
	self           znode.ID         // our own ID
...
func newDialScheduler(config dialConfig, it znode.Iterator, setupFunc dialSetupFunc) *dialScheduler {
	d := &dialScheduler{
		dialConfig:  config.withDefaults(),
		setupFunc:   setupFunc,
		dialing:     make(map[znode.ID]*dialTask),
		static:      make(map[znode.ID]*dialTask),
		peers:       make(map[znode.ID]struct{}),
		doneCh:      make(chan *dialTask),
		nodesIn:     make(chan *znode.Node),
		addStaticCh: make(chan *znode.Node),
		remStaticCh: make(chan *znode.Node),
...
func (d *dialScheduler) addStatic(n *znode.Node) {
	select {
...
func (d *dialScheduler) removeStatic(n *znode.Node) {
	select {
...
func (d *dialScheduler) loop(it znode.Iterator) {
	var (
		nodesCh    chan *znode.Node
...
func (d *dialScheduler) readNodes(it znode.Iterator) {
	defer d.wg.Done()
...
func (d *dialScheduler) checkDial(n *znode.Node) error {
	if n.ID() == d.self {
		return errSelf
	}
	if n.IP() != nil && n.TCP() == 0 {
...
func (d *dialScheduler) updateStaticPool(id znode.ID) {
	task, ok := d.static[id]
...
func newDialTask(dest *znode.Node, flags connFlag) *dialTask {
...
func (t *dialTask) dial(d *dialScheduler, dest *znode.Node) error {
