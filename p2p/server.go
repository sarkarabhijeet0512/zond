...
import (
	...
	"github.com/theQRL/zond/p2p/nat"
...
type Config struct {
	...
	BootstrapNodes []*znode.Node

	// BootstrapNodesV5 are used to establish connectivity
	// with the rest of the network using the V5 discovery
	// protocol.
	BootstrapNodesV5 []*znode.Node `toml:",omitempty"`

	// Static nodes are used as pre-configured connections which are always
	// maintained and re-connected on disconnects.
	StaticNodes []*znode.Node

	// Trusted nodes are used as pre-configured connections which are always
	// allowed to connect, even above the peer limit.
	TrustedNodes []*znode.Node
...
type Server struct {
	...
	discmix   *znode.FairMix
	dialsched *dialScheduler

	// Channels into the run loop.
	quit                    chan struct{}
	addtrusted              chan *znode.Node
	removetrusted           chan *znode.Node
...
type peerOpFunc func(map[znode.ID]*Peer)
...
func (srv *Server) LocalNode() *znode.LocalNode {
...
func (srv *Server) Peers() []*Peer {
	var ps []*Peer
	srv.doPeerOp(func(peers map[znode.ID]*Peer) {
		for _, p := range peers {
			ps = append(ps, p)
		}
	})
...
func (srv *Server) PeerCount() int {
	var count int
	srv.doPeerOp(func(ps map[znode.ID]*Peer) {
		count = len(ps)
	})
	return count
}
...
func (srv *Server) AddPeer(node *znode.Node) {
...
func (srv *Server) RemovePeer(node *znode.Node) {
	var (
		ch  chan *PeerEvent
...
func (srv *Server) AddTrustedPeer(node *znode.Node) {
	select {
...
func (srv *Server) RemoveTrustedPeer(node *znode.Node) {
	select {
...
func (srv *Server) Start() (err error) {
	...
	srv.addtrusted = make(chan *znode.Node)
	srv.removetrusted = make(chan *znode.Node)
...
func (srv *Server) setupLocalNode() error {
	...
	db, err := znode.OpenDB(srv.Config.NodeDatabase)
	if err != nil {
		return err
	}
	srv.nodedb = db
	srv.localnode = znode.NewLocalNode(db, srv.PrivateKey)
...
func (srv *Server) setupDiscovery() error {
	srv.discmix = znode.NewFairMix(discmixTimeout)
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		srv.localnode.Set(znr.TCP(tcp.Port))
...
func (srv *Server) run() {
	...
	var (
		peers        = make(map[znode.ID]*Peer)
		inboundCount = 0
		trusted      = make(map[znode.ID]bool, len(srv.TrustedNodes))
	)
...
func (srv *Server) postHandshakeChecks(peers map[znode.ID]*Peer, inboundCount int, c *conn) error {
	switch {
...
func (srv *Server) addPeerChecks(peers map[znode.ID]*Peer, inboundCount int, c *conn) error {
...
func (srv *Server) SetupConn(fd net.Conn, flags connFlag, dialDest *znode.Node) error {
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *znode.Node) error {
...
func nodeFromConn(pubkey *ecdsa.PublicKey, conn net.Conn) *znode.Node {
	...
	return znode.NewV4(pubkey, ip, port, port)
}
...
type NodeInfo struct {
	...
	Enode string `json:"znode"` // Enode URL for adding this peer from remote peers
	ENR   string `json:"znr"`   // Ethereum Node Record
