...
import (
	...
	"github.com/theQRL/zond/validator/db"
	"github.com/theQRL/zond/validator/db/kv"
	"github.com/theQRL/zond/validator/slashing-protection-history/format"
)
...
func ImportStandardProtectionJSON(ctx context.Context, validatorDB db.Database, r io.Reader) error {
	...
	attestingHistoryByPubKey := make(map[[dilithium.PKSizePacked]byte][]*kv.AttestationRecord)
	proposalHistoryByPubKey := make(map[[dilithium.PKSizePacked]byte]kv.ProposalHistoryForPubkey)
	...
	slashablePublicKeys := make([][dilithium.PKSizePacked]byte, 0, len(slashableAttesterKeys)+len(slashableProposerKeys))
...
//  }
func parseBlocksForUniquePublicKeys(data []*format.ProtectionData) (map[[dilithium.PKSizePacked]byte][]*format.SignedBlock, error) {
	signedBlocksByPubKey := make(map[[dilithium.PKSizePacked]byte][]*format.SignedBlock)
...
//  }
func parseAttestationsForUniquePublicKeys(data []*format.ProtectionData) (map[[dilithium.PKSizePacked]byte][]*format.SignedAttestation, error) {
	signedAttestationsByPubKey := make(map[[dilithium.PKSizePacked]byte][]*format.SignedAttestation)
	for _, validatorData := range data {
		pubKey, err := PubKeyFromHex(validatorData.Pubkey)
		if err != nil {
			return nil, fmt.Errorf("%s is not a valid public key: %w", validatorData.Pubkey, err)
...
func filterSlashablePubKeysFromBlocks(_ context.Context, historyByPubKey map[[dilithium.PKSizePacked]byte]kv.ProposalHistoryForPubkey) [][dilithium.PKSizePacked]byte {
	...
	slashablePubKeys := make([][dilithium.PKSizePacked]byte, 0)
...
func filterSlashablePubKeysFromAttestations(
	...
	signedAttsByPubKey map[[dilithium.PKSizePacked]byte][]*kv.AttestationRecord,
) ([][dilithium.PKSizePacked]byte, error) {
	slashablePubKeys := make([][dilithium.PKSizePacked]byte, 0)
...
func transformSignedAttestations(pubKey [dilithium.PKSizePacked]byte, atts []*format.SignedAttestation) ([]*kv.AttestationRecord, error) {
