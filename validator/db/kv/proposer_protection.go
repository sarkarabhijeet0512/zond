...
func (s *Store) ProposedPublicKeys(ctx context.Context) ([][dilithium.PKSizePacked]byte, error) {
	ctx, span := trace.StartSpan(ctx, "Validator.ProposedPublicKeys")
	defer span.End()
	var err error
	proposedPublicKeys := make([][dilithium.PKSizePacked]byte, 0)
	err = s.view(func(tx *bolt.Tx) error {
		bucket := tx.Bucket(historicProposalsBucket)
		return bucket.ForEach(func(key []byte, _ []byte) error {
			pubKeyBytes := [dilithium.PKSizePacked]byte{}
...
func (s *Store) ProposalHistoryForSlot(ctx context.Context, publicKey [dilithium.PKSizePacked]byte, slot types.Slot) ([32]byte, bool, error) {
...
func (s *Store) ProposalHistoryForPubKey(ctx context.Context, publicKey [dilithium.PKSizePacked]byte) ([]*Proposal, error) {
...
func (s *Store) SaveProposalHistoryForSlot(ctx context.Context, pubKey [dilithium.PKSizePacked]byte, slot types.Slot, signingRoot []byte) error {
...
func (s *Store) LowestSignedProposal(ctx context.Context, publicKey [dilithium.PKSizePacked]byte) (types.Slot, bool, error) {
...
// If no data exists, a boolean of value false is returned.
func (s *Store) HighestSignedProposal(ctx context.Context, publicKey [dilithium.PKSizePacked]byte) (types.Slot, bool, error) {
