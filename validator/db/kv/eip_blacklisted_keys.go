...
func (s *Store) EIPImportBlacklistedPublicKeys(ctx context.Context) ([][dilithium.PKSizePacked]byte, error) {
	ctx, span := trace.StartSpan(ctx, "Validator.EIPImportBlacklistedPublicKeys")
	defer span.End()
	var err error
	publicKeys := make([][dilithium.PKSizePacked]byte, 0)
...
func (s *Store) SaveEIPImportBlacklistedPublicKeys(ctx context.Context, publicKeys [][dilithium.PKSizePacked]byte) error {
