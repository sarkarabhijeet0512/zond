...
type AttestationRecord struct {
	PubKey      [dilithium.PKSizePacked]byte
	Source      types.Epoch
	Target      types.Epoch
	SigningRoot [32]byte
}
...
func (s *Store) AttestationHistoryForPubKey(ctx context.Context, pubKey [dilithium.PKSizePacked]byte) ([]*AttestationRecord, error) {
...
func (s *Store) AttestedPublicKeys(ctx context.Context) ([][dilithium.PKSizePacked]byte, error) {
	ctx, span := trace.StartSpan(ctx, "Validator.AttestedPublicKeys")
	defer span.End()
	var err error
	attestedPublicKeys := make([][dilithium.PKSizePacked]byte, 0)
	err = s.view(func(tx *bolt.Tx) error {
		bucket := tx.Bucket(pubKeysBucket)
...
func (s *Store) SigningRootAtTargetEpoch(ctx context.Context, pubKey [dilithium.PKSizePacked]byte, target types.Epoch) ([32]byte, error) {
...
func (s *Store) LowestSignedSourceEpoch(ctx context.Context, publicKey [dilithium.PKSizePacked]byte) (types.Epoch, bool, error) {
...
// If no data exists, returning 0 is a sensible default.
func (s *Store) LowestSignedTargetEpoch(ctx context.Context, publicKey [dilithium.PKSizePacked]byte) (types.Epoch, bool, error) {
