...
type Backend interface {
	...
	PeerInfo(id znode.ID) interface{}
...
func MakeProtocols(backend Backend, dnsdisc znode.Iterator) []p2p.Protocol {
	// Filter the discovery iterator for nodes advertising snap support.
	dnsdisc = znode.Filter(dnsdisc, func(n *znode.Node) bool {
		var snap znrEntry
	...
	for i, version := range ProtocolVersions {
		...
		protocols[i] = p2p.Protocol{
			...
			PeerInfo: func(id znode.ID) interface{} {
				return backend.PeerInfo(id)
			},
			Attributes:     []znr.Entry{&znrEntry{}},
