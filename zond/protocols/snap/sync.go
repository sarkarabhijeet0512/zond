...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	for i, hash := range res.hashes {
		...
		slim := snapshot.SlimAccountRLP(res.accounts[i].Nonce, res.accounts[i].Balance, res.accounts[i].StakeBalance, res.accounts[i].PendingStakeBalance, res.accounts[i].Root, res.accounts[i].CodeHash, res.accounts[i].OTSBitfield)
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	if len(paths) == 1 {
		...
		blob := snapshot.SlimAccountRLP(account.Nonce, account.Balance, account.StakeBalance, account.PendingStakeBalance, account.Root, account.CodeHash, account.OTSBitfield)
