...
func (api *API) blockByNumberAndHash(ctx context.Context, number rpc.BlockNumber, hash common.Hash) (*protos.Block, error) {
	b, err := api.blockByNumber(ctx, number)
	if err != nil {
		return nil, err
	}
	if common.BytesToHash(b.Header.Hash) == hash {
		return b, nil
	}
